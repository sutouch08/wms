...
{
  private $tb = "production_issue";
  private $td = "production_issue_details";
  private $tm = "production_issue_batch";
  ...
  {
    $rs = $this->db->where('issue_code', $code)->get($this->td);
  ...
  {
    $rs = $this->db->where('issue_detail_id', $detail_id)->get($this->tm);
  ...
  public function update_details($code, array $ds = array())
  {
    if( ! empty($ds))
    {
      return $this->db->where('issue_code', $code)->update($this->td, $ds);
    }
  ...
  public function update_batch_rows($detail_id , array $ds = array())
  {
    if( ! empty($ds))
    {
      return $this->db->where('issue_detail_id', $detail_id)->update($this->tm, $ds);
    }
  ...
  public function update_batches($code, array $ds = array())
  {
    if( ! empty($ds))
    {
      return $this->db->where('issue_code', $code)->update($this->tm, $ds);
    }
  ...
  {
    $count = $this->ms->where('U_ECOMNO', $code)->where('CANCELED', 'N')->count_all_results('OIGE');

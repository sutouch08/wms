...
class Delivery_order_model extends CI_Model
{
  ...
  public function get_list(array $ds = array(), $perpage = 20, $offset = 0, $state = 7)
  {
    ...
    ->select('channels_code, payment_code, date_add, shipped_date, shipping_code')
    ->select('user, doc_total, inv_code, empID, empName, is_cancled, is_hold, is_cancled, shop_id')
    ->where('state', $state);

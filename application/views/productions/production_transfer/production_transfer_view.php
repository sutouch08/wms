    ...
    <button type="button" class="btn btn-white btn-default top-btn" onclick="goBack()"><i class="fa fa-arrow-left"></i> Back</button>
		...
		<div class="btn-group">
			...
			<ul class="dropdown-menu dropdown-menu-right">
				...
				<?php endif; ?>
				<?php if(($this->pm->can_add OR $this->pm->can_edit) && $doc->Status == 'R') : ?>
					<li class="success">
						<a href="javascript:close('<?php echo $doc->code; ?>')"><i class="fa fa-check"></i> Close Document</a>
					</li>
				<?php endif; ?>
				<?php if($this->pm->can_edit && ($doc->Status == 'R' OR $doc->Status == 'C')) : ?>
					<li class="purple">
						<a href="javascript:rollBack('<?php echo $doc->code; ?>')"><i class="fa fa-history"></i> Rollback Status</a>
					</li>
				<?php endif; ?>
				<li class="info">
					<a href="javascript:printPickList('<?php echo $doc->code; ?>')"><i class="fa fa-print"></i> Print Pick List</a>
				</li>
...
<div class="row">
  <!-- Left column -->
  <?php $this->load->view('productions/production_transfer/production_transfer_view_header_left'); ?>
  <!-- Right Column -->
  <?php $this->load->view('productions/production_transfer/production_transfer_view_header_right'); ?>
</div>
<hr class="padding-5">
<?php if($doc->Status == 'D') { $this->load->view('cancle_watermark'); } ?>

<?php $this->load->view('productions/production_transfer/production_transfer_view_details'); ?>
...
<script src="<?php echo base_url(); ?>scripts/productions/production_transfer/production_transfer_view.js?v=<?php echo date('Ymd'); ?>"></script>

...
<div class="row">
  <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 padding-5 table-responsive">
    <table class="table table-striped border-1" id="table-listing" style="min-width:1100px;">
      <tr class="font-size-11">
				<th class="fix-width-60 text-center"></th>
        <th class="fix-width-50 text-center">#</th>
				<th class="fix-width-60">Status</th>
        <th class="fix-width-80">Date</th>
    ...
    <?php foreach($data as $rs) : ?>
      <tr class="font-size-11 pointer" id="row-<?php echo $rs->id; ?>" style="<?php echo issue_status_color($rs->Status); ?>">
        ...
        <td class="middle text-center no" onclick="viewDetail('<?php echo $rs->code; ?>')"><?php echo $no; ?></td>
				<td class="middle" onclick="viewDetail('<?php echo $rs->code; ?>')"><?php echo production_issue_status_text($rs->Status); ?></td>
        <td class="middle" onclick="viewDetail('<?php echo $rs->code; ?>')"><?php echo thai_date($rs->date_add, FALSE); ?></td>
        <td class="middle"><a target="_blank" href="<?php echo $this->home; ?>/view_detail/<?php echo $rs->code; ?>" style="color:inherit;"><?php echo $rs->code; ?></a></td>
        ...
        <td class="middle" onclick="viewDetail('<?php echo $rs->code; ?>')"> <?php echo $rs->user; ?></td>
				<td class="middle" onclick="viewDetail('<?php echo $rs->code; ?>')"> <?php echo $rs->remark; ?></td>
      </tr>
    <?php  $no++; ?>
    <?php endforeach; ?>
    <?php else : ?>

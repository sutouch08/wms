...
<form id="searchForm" method="post" action="<?php echo current_url(); ?>">
	<div class="row">
		...
		<div class="col-lg-3 col-md-4 col-sm-4 col-xs-6 padding-5">
			<label>คลัง</label>
			<select class="form-control input-sm filter" name="warehouse_code" id="warehouse">
...
<form id="export-form" action="<?php echo $this->home; ?>/export_filter" method="post">
	...
	<input type="hidden" name="range" id="ex-range" />
			...
			<tbody>
				<?php if (!empty($data)) : ?>
					...
					<?php foreach ($data as $rs) : ?>
						<?php $stateName = ''; ?>
						<?php if (! isset($state[$rs->order_code])) : ?>
							<?php $state[$rs->order_code] =  $this->prepare_list_model->get_order_state($rs->order_code); ?>
						<?php endif; ?>
						<?php $stateName = get_state_name($state[$rs->order_code]); ?>
						...
						<tr class="font-size-11">
							...
							<td class="text-center"><?php echo $stateName; ?></td>

...
<div class="row">
	...
	<div class="col-lg-8 col-md-8 col-sm-8 col-xs-12 padding-5 text-right hidden-xs">
		<button type="button" class="btn btn-white btn-primary top-btn" onclick="genPickList()">พิมพ์ใบจัด</button>
		<button type="button" class="btn btn-white btn-warning top-btn" onclick="goBack()"><i class="fa fa-external-link-square"></i> รอจัด</button>
		<?php if ($this->pm->can_edit) : ?>
			<button type="button" class="btn btn-white btn-primary top-btn" onclick="express_order()"><i class="fa fa-qrcode"></i>&nbsp; สแกนออเดอร์</button>
			<div class="btn-group">
...
<form id="searchForm" method="post" action="<?php echo current_url(); ?>">
	<div class="row filter-pad move-out" id="filter-pad">
		...
		<div class="col-lg-2-harf col-md-4-harf col-sm-3 col-xs-6 padding-5 fi">
			<label>พนักงาน</label>
		...
		<div class="col-lg-3 col-md-4 col-sm-3 col-xs-6 padding-5 fi">
			<label>คลัง</label>
		...
		<div class="col-lg-2 col-md-2-harf col-sm-3 col-xs-6 padding-5 fi">
		...
		<div class="col-lg-1 col-md-1-harf col-sm-1-harf col-xs-3 padding-5 fi">
			...
			<select class="form-control input-sm" name="is_backorder">
				<option value="all">ทั้งหมด</option>
				<option value="1" <?php echo is_selected($is_backorder, '1'); ?>>Yes</option>
				<option value="0" <?php echo is_selected($is_backorder, '0'); ?>>No</option>
			</select>
		</div>
		...
		<div class="col-lg-2 col-md-3 col-sm-3 col-xs-6 padding-5 fi">
			<label>วันที่</label>
...
<div class="row">
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 padding-5 table-responsive">
		<table class="table table-hover border-1 no-border-xs table-process table-narrow">
			<thead>
					...
					<?php foreach ($orders as $rs) : ?>
						...
						<?php if (! empty($rs->pick_list_id)) : ?>
							<?php if (empty($pickList[$rs->pick_list_id])) : ?>
								<?php $pickList[$rs->pick_list_id] = $this->pick_list_model->get_code($rs->pick_list_id); ?>
							<?php endif; ?>
						<?php endif; ?>
						<?php $picklist_code = empty($rs->pick_list_id) ? '' : $pickList[$rs->pick_list_id]; ?>
						<tr id="row-<?php echo $rs->code; ?>" class="font-size-11 <?php echo $color; ?>">
							<td class="middle hidden-xs">
								...
								<?php if ($this->pm->can_delete) : ?>
									<button type="button" class="btn btn-white btn-mini btn-warning" onClick="pullBack('<?php echo $rs->code; ?>')">ดึงกลับ</button>
								<?php endif; ?>
							</td>
							<td class="middle hidden-xs">
								<label>
							...
							<td class="middle hidden-xs">
								<?php if (! empty($picklist_code)) : ?>
									<a target="_blank" href="<?php echo base_url(); ?>inventory/pick_list/view_detail/<?php echo $picklist_code; ?>"><?php echo $picklist_code; ?></a>
								<?php endif; ?>
							</td>

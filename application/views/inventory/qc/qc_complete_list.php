      ...
      <tr id="row-<?php echo $rs->id; ?>">
        ...
        <td class="middle text-right">
          <?php if(($rs->qc > $rs->prepared OR $rs->qc > $rs->order_qty) && $this->pm->can_delete) : ?>
            <button type="button" class="btn btn-minier btn-warning must-edit" onclick="showEditOption('<?php echo $order->code; ?>', '<?php echo $rs->product_code; ?>')">
              <i class="fa fa-pencil"></i> แก้ไข
            </button>
          <?php endif; ?>
          <button
            type="button"
            class="btn btn-minier btn-default btn-pop"
            data-container="body"
            data-toggle="popover"
            data-placement="left"
            data-trigger="focus"

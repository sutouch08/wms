...
<script src="<?php echo base_url(); ?>scripts/wms/soko_temp_delivery.js?v=<?php echo date('Ymd'); ?>"></script>
<script>
	...
	function rollBack(id) {
		$.ajax({
			url:BASE_URL + "rest/V1/soko_temp_delivery/rollback_status",
			type:"POST",
			cache:false,
			data:{
				"id" : id
	...
	function close_temp(id) {
		$.ajax({
			url:BASE_URL + "rest/V1/soko_temp_delivery/close_temp",
			type:"POST",
			cache:false,
			data:{
				"id" : id
	...
	function process() {
		load_in();
		$.ajax({
			url:BASE_URL + "auto/soko_auto_delivery/do_delivery",
			type:'GET',
			success:function(rs) {
				load_out();
				if(rs == 'success') {

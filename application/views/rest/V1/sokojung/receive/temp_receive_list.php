...
<script src="<?php echo base_url(); ?>scripts/wms/soko_temp_receive.js?v=<?php echo date('Ymd'); ?>"></script>
<script>
	function process() {
		load_in();
		$.ajax({
			url:BASE_URL + "auto/soko_auto_receive/do_receive",
			type:'GET',
			success:function(rs) {
				load_out();
				if(rs == 'success') {
	...
	function doReceive(id)
	{
		load_in();
		$.ajax({
			url:BASE_URL + "auto/soko_auto_receive/do_receive/"+id,
			type:'GET',
			success:function(rs) {
				load_out();
				if(rs == 'success') {

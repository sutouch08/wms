...
class Delivery_order extends PS_Controller
{
  ...
  public function confirm_order()
  {
    ...
    if($sc === TRUE)
    {
      ...
      if($order->state == 7)
      {
        ...
        if($sc === TRUE)
        {
          ...
          {
            $sc = FALSE;
            $this->error = "Change order state failed";
          }
    ...
    if($sc === TRUE)
    {
      ...
      {
        if(is_true(getConfig('WRX_API')) && is_true(getConfig('WRX_WEB_TRACKING_API')))
        {
          $this->load->library('wrx_web_api');

          if($this->wrx_web_api->send_tracking($order->reference, $order->shipping_code))
          {
            $this->orders_model->update($code, ['send_tracking' => 1, 'send_tracking_error' => NULL]);
          }
          else
          {
            $this->orders_model->update($code, ['send_tracking' => 3, 'send_tracking_error' => $this->wrx_web_api->error]);
          }
        }
      }
    }

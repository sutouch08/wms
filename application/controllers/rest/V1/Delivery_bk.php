...
class Delivery extends REST_Controller
{
	...
	public function create_post()
  {
	    ...
	    if(empty($order))
	    {
	      ...
	      $this->error = 'Invalid Order number';
	        ...
	        'message' => $this->error
				...
				$this->wms_error_logs_model->add(NULL, 'E', $this->error);
	      ...
	      $this->response($arr, 400);
			...
			if($sc === TRUE)
			{
		        ...
		        'message' => $this->error
					...
					$this->wms_error_logs_model->add($order->code, 'E', $this->error);
		    ...
		    {
		      $sc = FALSE;
		      $this->error = "No Items In Order List";
	        ...
	        $this->load->model('inventory/lend_model');
			 ...
			 $this->orders_model->change_state($order->code, 8);
				...
				foreach($details as $rs)
				{
					...
					$ds = $this->orders_model->get_order_detail($order->code, $rs->item);
					if(!empty($ds))
					{
					...
					{
						$sc = FALSE;
						$this->error = "Update failed : No Item Code '{$rs->item}' In Order List";
						$this->wms_error_logs_model->add($order->code, 'E', $this->error);
						$message = $this->error;
						break;
					}
    ...
    {
			$message = empty($message) ? "Update failed" : $message;
      $arr = array(
        'status' => FALSE,
        'message' => $message
      );

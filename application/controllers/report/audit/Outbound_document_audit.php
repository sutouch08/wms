      ...
      foreach($result as $rs)
      {
        $ds = array(
          ...
          'ix_code' => $rs->order_code,
					...
					'wms_type' => 'OB',
					'sap_code' => $rs->inv_code,
					'sap_type' => $SapDoc[$rs->role],
					'ix_state' => $stateName[$rs->state],
					'channels' => $channelsName[$rs->channels_code]
        );
  ...
  public function do_export()
  {
    ...
    if(!empty($result))
    {
      ...
      foreach($result as $rs)
      {
        ...
        $this->excel->getActiveSheet()->setCellValue('G'.$row, $rs->inv_code);
        ...
        $this->excel->getActiveSheet()->setCellValue('I'.$row, $stateName[$rs->state]);
				$this->excel->getActiveSheet()->setCellValue('J'.$row, $channelsName[$rs->channels_code]);

<?php
class Soko_auto_receive extends CI_Controller
{
  public $home;
	public $wms;
  public $mc;
  ...
  public function __construct()
  {
    ...
    $this->home = base_url().'auto/soko_auto_receive';
  ...
  public function index()
  {
		...
		$list = $this->soko_temp_receive_model->get_unprocess_list($limit);
    ...
    if($id !== NULL)
    {
      $list = $this->soko_temp_receive_model->get_temp_data($id);
    }
    else
    {
      $limit = 10;
    ...
    $sc = TRUE;
		...
		if( ! empty($order))
		{
      $date_add = getConfig('ORDER_SOLD_DATE') == 'D' ? $order->date_add : ( ! empty($order->shipped_date) ? $order->shipped_date : (empty($data->receive_date) ? now() : $data->receive_date));

			if($order->status == 1)
			{
				$sc = FALSE;
				$this->error = "Document already received";
				$this->soko_temp_receive_model->update_status($order->code, 3, $this->error);
			}
			...
			{
				$details = $this->soko_temp_receive_model->get_details($data->id);

				if(!empty($details))
				{
					$this->db->trans_begin();

					$warehouse_code = getConfig('SOKOJUNG_WAREHOUSE'); //--- คลัง
					$zone_code = getConfig('SOKOJUNG_ZONE'); //--- โซน wms
                ...
                foreach($rows as $row)
                {
                  if($temp_qty > 0)
                  {
					...
					if($sc === TRUE)
					{
						...
						$this->soko_temp_receive_model->update_status($order->code, 1, 'success');
					}
					else
					{
				...
				{
					$sc = FALSE;
					$this->soko_temp_receive_model->update_status($order->code, 3, "No Items In Order List");
				}
			}
		}
		else
		...
		if(!empty($order))
		{
			...
			else
			{
				...
				if(!empty($details))
				{
					...
					$warehouse_code = ! empty($order->warehouse_code) ? $order->warehouse_code : getConfig('SOKOJUNG_WAREHOUSE'); //--- คลัง wms
					$zone_code = ! empty($order->zone_code) ? $order->zone_code : getConfig('SOKOKJUNG_ZONE'); //--- โซน wms
	...
	private function check_return($data)
	{
		...
		if(!empty($code))
		{
			if(! $this->soko_temp_receive_model->update_status($code, 1, NULL))
			{
				$sc = FALSE;
			}
		}

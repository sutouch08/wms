...
class Production_transfer extends PS_Controller
{
  ...
  public function add()
  {
    ...
    if( ! empty($ds) && ! empty($ds->date_add) && ! empty($ds->fromWhsCode) && ! empty($ds->toWhsCode) && ! empty($ds->rows))
    {
      ...
      if($sc === TRUE)
      {
        foreach($ds->rows as $rs)
        {
          ...
          if($rs->hasBatch == 0 && (empty($rs->fromBinCode) OR empty($rs->fromWhsCode)))
          {
            $sc = FALSE;
            $this->error = "Missing From Whs or From Bin for line item {$rs->ItemCode}";
          }

          if(empty($rs->toWhsCode) OR empty($rs->toBinCode))
          {
            $sc = FALSE;
